...
//
// Used to proxy direct CDN URLs (video / image) to the browser with proper
// Content-Disposition so the browser triggers a Save dialog.
//
// NOTE: Carousel ZIPs are served directly from /instagram/post — they do NOT
// go through this proxy. This proxy is only for single video/image CDN URLs.
//
export async function GET(req) {
  ...
  // Instagram videos are served from fbcdn.net (Facebook CDN)
  const allowedHosts = [
    "cdninstagram.com",
    "fbcdn.net", // PRIMARY — Facebook CDN serves all IG videos
    "instagram.com",
  ...
  try {
    ...
    if (data.byteLength < 500)
      return new Response(
        "File too small — upstream likely blocked the request",
        {
          status: 502,
        },
      );
