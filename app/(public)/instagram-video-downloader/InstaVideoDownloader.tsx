...
function isValidInstagramUrl(val: string): boolean {
  return (
    /instagram\.com\/(p|reel|reels|tv|stories)\/[A-Za-z0-9_-]+/.test(val) ||
    /instagram\.com\/[A-Za-z0-9_.]+\/(reel|reels|p)\/[A-Za-z0-9_-]+/.test(val)
  );
}
...
export default function InstaRipDownloader() {
  ...
  const handleDownload = async () => {
    ...
    setErrorMsg("");
    try {
      const endpoint = "/api/instagram-video"; // handles video, image, carousel
